...
Regarding the validation stage (see \sref{validation}), the considered
...
stages, takes around 0 hours or 0 days. During this process, we run up to four
training sessions in parallel, which typically keeps all the eight \up{CPU}s
busy. It should be noted that, due to the fact that the training, validation,
and testing data sets have been cached on disk as a result of our
data-processing pipeline described in \sref{data}, individual training sessions
...
hyperparameters: $c = 0$, $u = 0$, and $p = 0$. However, one can also observe
in \tref{validation} that there are several other candidates that are very close
to the best one. In general, deeper and wider architectures tend to outperform
shallower and narrower ones (the depth and breadth are measured by $c$ and $u$,
respectively), which is expected. We also would like to note that the \up{MSE}s
tend to plateau as the number of training steps increases (not shown), which
suggests a limit to what can be extracted from the data with the model.
...
decrease in accuracy for a considerable memory saving. For example, the zeroth
best configuration requires 0\% less memory than the first one.
...
elaborate sequential testing procedure takes around 0 hours from start to
finish.

In order to assess better the accuracy of our model, we employ an alternative
one, which we shall refer to as the reference model. The reference model is
based on random walk. It postulates that the best prediction of what will happen
tomorrow is what happens today: the next value of a resource-usage trace is
estimated to be the current one. This strategy results in four identical
