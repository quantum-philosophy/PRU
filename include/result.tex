...
\input{include/assets/figures/traces}
...
It can be seen in \fref{traces}, which shows a histogram of the traces' lengths,
that such traces constitute around 74\% of the total number of traces (around 18
out of 24 million).
...
The training stage (\sref{training}) is configured as follows. Based on
\fref{traces}, 10 buckets are used according to the rule: $l < 6 < 7 < 8 < 9 <
10 < 15 < 20 < 30 < 40 \leq 50$. The batch size $b$ is set to 64. The
optimization algorithm employed for minimizing of the loss function is Adam
...
$u \in \{100, 200, 400, 800, 1600\}$, and $p \in \{0, 0.1\}$, which yields 50
...
settings. The maximum budget for one configuration is set to two training
epochs, which correspond to $2 \times 0.7 \times 2 \times 10^6 = 2.8 \times
10^6$ traces or $2.8 \times 10^6 \div 64 = 43\,750$ training steps.
...
However, one can observe in \fref{validation} that there are several other
...
\up{MSE} of our best trained model (the blue line) as well as the one of the
reference model (the red line) with respect to $X_3$. The magnitude of our
model's errors suggests that the amount of regularity present in the data is not
sufficient to make the resource-usage predictions extremely accurate.
Nevertheless, it can be seen in \fref{testing} that, relative to the reference
model, our model provides an error reduction of approximately 30\% at each of
the four future time moments. This observation suggests that a certain structure
does exist, and that it can be identified and utilized in order to make educated
predictions.
